...
mod tests {
    ...
    #[test]
    fn builds_full_vault_and_updates_removes_and_renames() {
        let dir = tempdir().unwrap();
        let vault = dir.path().join("vault");
        fs::create_dir(&vault).unwrap();
        fs::create_dir(vault.join("work")).unwrap();
        ...
        rename_path(&mut conn, &vault, &alpha, &renamed, None).unwrap();
        let renamed_row = query::by_id(&conn, &note_id(&alpha)).unwrap().unwrap();
        assert_eq!(renamed_row.path, renamed);
    }

    #[test]
    fn emits_progress_while_building() {
        let dir = tempdir().unwrap();
        let vault = dir.path().join("vault");
        fs::create_dir(&vault).unwrap();
        for index in 0..120 {
        ...
        let sink = move |event| events_clone.lock().unwrap().push(event);
        let mut conn = open_index_at(&dir.path().join("index.sqlite")).unwrap();
    ...
    #[test]
    fn builds_one_thousand_notes_under_budget_on_dev_machine() {
        let dir = tempdir().unwrap();
        let vault = dir.path().join("vault");
        fs::create_dir(&vault).unwrap();
        ...
        let update_path = vault.join("note-999.md");
        fs::write(&update_path, "# Note 999 changed\n#tag").unwrap();
