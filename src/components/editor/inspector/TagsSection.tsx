...
export function TagsSection() {
  ...
  const handleAdd = useCallback(
    (tag: string) => {
      ...
      const current = Array.isArray(editor.frontmatter.tags)
        ? (editor.frontmatter.tags as string[])
        : [];
  ...
  return (
    <section>
      ...
      {tags.length === 0 && !picking && (
        <p className="text-[11px] text-[var(--color-cork-subtle)]">No tags on this note</p>
      )}
      {tags.length > 0 && (
        <div className="flex flex-wrap gap-1.5">
          {tags.map((tag) => (
            <span
              key={tag}
              className="group/tag inline-flex items-center gap-1 rounded-md bg-[var(--color-cork-tag-soft)] px-2 py-0.5 text-[11px] font-medium text-[var(--color-cork-tag)]"
            >
              <Tag size={10} />
              {tag}
              <button
                onClick={() => handleRemove(tag)}
                className="ml-0.5 rounded-full p-0.5 opacity-0 transition-opacity hover:bg-[var(--color-cork-tag)]/20 group-hover/tag:opacity-100"
                title="Remove tag"
              >
      ...
      {picking && (
        <div className="mt-2">
          ...
          {suggestions.length > 0 ? (
              ...
              {suggestions.map((tag, i) => (
                <li key={tag}>
                  <button
                    ...
                    className={`flex w-full items-center gap-1.5 px-2 py-1 text-left text-[11px] ${i === selectedIndex ? "bg-[var(--color-cork-tag-soft)] text-[var(--color-cork-tag)]" : "text-[var(--color-cork-ink)] hover:bg-[var(--color-cork-panel-2)]"}`}
                  >
                    <Tag size={10} className="text-[var(--color-cork-tag)]" />
                    {tag}
                  </button>
                </li>
              ))}
            ...
            <p className="mt-1 px-1 text-[11px] text-[var(--color-cork-subtle)]">
              {search.trim() ? "No tags found" : "No tags available"}
            </p>
          )}
        </div>
      )}
