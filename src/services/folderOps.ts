...
export const folderOps = {
  create: (input: { parent: string; name: string }) => client.folders.create(input),
  rename: (input: { oldPath: string; newName: string }) => client.folders.rename(input),
  move: (input: { srcPath: string; destParent: string }) => client.folders.move(input),
