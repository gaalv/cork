...
type VaultListItemProps = {
  ...
  onMissing: (vault: RecentVault) => void;
};

export function VaultListItem({ vault, activePath, onSelect, onMissing }: VaultListItemProps) {
  const active = vault.path === activePath;
  return (
    <button
      type="button"
      disabled={active}
      onClick={() => (vault.missing ? onMissing(vault) : onSelect(vault.path))}
    ...
    >
      ...
      {vault.missing ? <span className="shrink-0 text-[11px] text-amber-600">Missing</span> : null}
