...
describe("ListPane", () => {
  it("renders the scope label and notes for the recent shortcut", async () => {
    ...
    expect(screen.getByTestId("triage-list-count").textContent).toBe("2 notes");
  });
  ...
  it("uses byTag for tag selections", async () => {
  ...
  it("filters by folder prefix for folder selections", async () => {
    ...
    render(<ListPane />);
    await waitFor(() => expect(screen.getByTestId("triage-list-count").textContent).toBe("2 notes"));
  });
