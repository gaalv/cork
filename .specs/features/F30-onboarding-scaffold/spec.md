...
When Noxe opens an empty vault (or one that has never seen our scaffold), it
...
- **R1** When a vault is opened that has zero `.md` files at the root **and**
  has no `.noxe/scaffold.json` marker, the backend writes a curated set of
  example notes and folders, then writes the marker so it never repeats.
- **R2** Scaffold contents (paths relative to vault root):
  - `Welcome.md` — landing note explaining Noxe's main features (wikilinks,
  ...
  - `Daily/2026-05-08.md` — example daily note explaining the daily-note
    pattern.
  - `Projects/Noxe Showcase.md` — multi-section example with headings, a
  ...
  - `Cheatsheet.md` — compact reference of the markdown extensions Noxe
    supports.
...
- **R5** The frontend toasts "Welcome to Noxe — example notes added" when
  the scaffold runs.
- **R6** A few seed `.noxe/todos.json` entries are added so the new
...
- The marker file `.noxe/scaffold.json` is created and ignored by git
  (already covered by `.noxe/cache/` ignore pattern adjacency — verify).
