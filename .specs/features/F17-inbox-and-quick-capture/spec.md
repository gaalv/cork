...
3. **No quick capture.** When something noteworthy comes up in a call, the user has to focus the app, click around, then type. We want a single keystroke that captures, even when Noxe is minimized.
...
- A **macOS menubar tray icon** with at minimum: Quick capture / Show Noxe / Quit.
...
- **FR7.** A macOS tray icon is created at startup. Left-click toggles main window visibility. Right-click (or click on Linux/Win) opens a context menu: **Quick capture** (`CmdOrCtrl+Shift+I` accelerator label), separator, **Show Noxe**, **Quit Noxe**.
- **FR8.** When the user clicks the OS window close button (`X`), the window is hidden instead of being destroyed. Reopening (tray, shortcut, or `Show Noxe`) shows the existing window with state intact.
...
- **FR10.** "Quit Noxe" from the tray (and `Cmd+Q`) actually quit the process, including unregistering the global shortcut.
...
- Press `Cmd+Shift+I` while in a different app → Noxe comes to the front with a brand new empty note in `Inbox/` ready to type.
- Tray "Quit Noxe" → process exits.
...
- **Global shortcut collision.** `CmdOrCtrl+Shift+I` is used by Chrome to open DevTools; harmless when Noxe owns it system-wide. Risk: user has another app already using it. Acceptable for v1 since shortcut config is deferred.
